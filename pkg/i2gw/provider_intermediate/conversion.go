...
func ToEmitterIR(pIR ProviderIR) emitterir.EmitterIR {
	...
	for k, v := range pIR.HTTPRoutes {
		...
		ruleBackendSources := make([][]emitterir.BackendSource, len(v.RuleBackendSources))
		for i, ruleSources := range v.RuleBackendSources {
			ruleBackendSources[i] = make([]emitterir.BackendSource, len(ruleSources))
			for j, src := range ruleSources {
				ruleBackendSources[i][j] = emitterir.BackendSource{
					Ingress:        src.Ingress,
					Path:           src.Path,
		...
		eIR.HTTPRoutes[k] = emitterir.HTTPRouteContext{
