...
func (e *Emitter) ToKgatewayResources(ir emitterir.EmitterIR, gwResources *i2gw.GatewayResources) {
	e.EmitBackendProtocol(ir, gwResources)
	e.EmitBackendTLS(ir, gwResources)
	e.EmitBuffer(ir)
	// e.EmitTimeouts(ir)
	e.EmitRateLimit(ir)
	e.EmitCors(ir, gwResources)
	e.EmitPathRewrite(ir, gwResources)
	...
	for _, httpListenerPolicy := range e.builderMap.HTTPListenerPolicies {
		kgatewayObjs = append(kgatewayObjs, httpListenerPolicy)
	}
	...
	for _, backend := range e.builderMap.Backends {
		kgatewayObjs = append(kgatewayObjs, backend)
	}
