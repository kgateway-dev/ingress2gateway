...
// GatewayIR is the emitter-specific IR for kgateway.
type GatewayIR struct{}

// HTTPRouteIR contains kgateway-specific fields for HTTPRoute.
type HTTPRouteIR struct {
	// Policies keyed by source Ingress name.
	Policies map[string]Policy

	// RegexLocationForHost is true when kgateway would enforce the "~*" (case-insensitive)
	// regex location modifier for ALL paths under a host.
	RegexLocationForHost *bool

	// RegexForcedByUseRegex is true when RegexLocationForHost is true.
	RegexForcedByUseRegex bool

	// RegexForcedByRewrite is true when RegexLocationForHost is true.
	RegexForcedByRewrite bool
}

// ServiceIR contains kgateway-specific fields for Service.
...
type CorsPolicy struct {
	// Enable indicates whether CORS is enabled.
	Enable bool

	// AllowOrigin controls what is the accepted Origin for CORS.
	AllowOrigin []string

	// AllowCredentials controls if credentials can be passed during CORS operations.
	// When nil, a value has not been specified.
	AllowCredentials *bool

	// AllowHeaders ontrols which headers are accepted. Values are stored as raw header names;
	// case-insensitivity is handled by consumers.
	AllowHeaders []string

	// ExposeHeaders controls which headers are exposed to the client. Values are header names trimmed
	// of surrounding whitespace but otherwise case-preserving.
	ExposeHeaders []string

	// AllowMethods controls which methods are accepted. Values are stored as raw method names; consumers
	// can normalize/validate.
	AllowMethods []string

	// MaxAge defines the maximum age, in seconds and controls how long preflight requests can be cached.
	// When nil, a value has not been specified.
	MaxAge *int32
}

// ExtAuthPolicy defines an external authentication policy.
...
// BasicAuthPolicy defines a basic authentication policy.
...
// SessionAffinityPolicy defines a session affinity policy.
...
// BackendTLSPolicy defines a backend TLS policy.
...
// Policy describes all per-Ingress policy knobs (buffer, CORS, etc.).
type Policy struct {
	...
	// Cors defines the CORS policy.
	Cors *CorsPolicy

	// RateLimit is a generic rate limit policy.
	...
	// SSLRedirect indicates whether SSL redirect is enabled. When true, requests should be redirected to HTTPS.
	SSLRedirect *bool

	// RewriteTarget rewrites the path in the request to the path expected by the service.
	RewriteTarget *string

	// UseRegexPaths indicates whether paths contributed by this ingress are regex patterns. When true
	// (and host-wide regex mode is enabled), paths contributed by this ingress must be treated as regex
	// patterns (i.e. NOT escaped as literals).
	UseRegexPaths *bool
...
type RateLimitPolicy struct {
	// Exactly one of RPS/RPM should be set.
...
// LoadBalancingStrategy represents the upstream load-balancing mode. Currently only round_robin
// is supported; other values are ignored.
